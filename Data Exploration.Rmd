...
Av_age <- Combined_data %>%
  select(DivisionNm, Population, `0-17`:`80+`) %>%
  mutate(`0-17` = round(`0-17`*Population), digits = 0,
         `18-34` = round(`18-34`*Population), digits = 0,
  ...
  pivot_longer(cols = `0-17`:`80+`, names_to = "Agegroup", values_to = "Freq") %>%
...
Av_age <- Av_age %>%
  ...
  group_by(DivisionNm) %>%
  summarise("Average Age" = sum(AgegroupFreq)/sum(Freq))


density.plot(Av_age, Av_age$`Average Age`, 20) +
  labs(x = "Average Age")
...
ggplot() +
  geom_polygon(data = fort_Divisions, aes(fill = `Average Age`, x = long, y = lat, group = group)) +
  theme_void() +
  coord_map() +
